...
namespace Mss.Data
{
    public static class LoadLabelFormatter
    {
        ...
        public static string LabelSku = "SKU";
        public static string LabelSeatDescription = "SEAT_DESCRIPTION";
        public static string LabelTopBarcode = "TOP_BARCODE";
        public static string LabelLeftBarcode = "LEFT_BARCODE";
        public static string LabelJobID = "JOB_ID";
        ...
        public static XLabelDataPairs Format(LoadItem loadItem, int groupIndex)
        {
            PalletItem palletItem = loadItem.Pallet;
            BroadcastItem broadcastItem = loadItem.Broadcast;

            DateTime now = DateTime.Now;
            string program = palletItem.Sku[1] == 'B' ? "U725" : "P703";
            string row = palletItem.VehicleRow == VehicleRow.Row1 ? "FRONT" : "REAR";


            //             return new XLabelDataPairs(Constant.LoadLabelName)
            //             {
            //                 {LabelTopBarcode, broadcastItem.BroadcastNumber.ToString(Constant.BroadcastNumberFormat)},
            //                 {LabelLeftBarcode, $"{broadcastItem.RotationText} {broadcastItem.Vin}"},
            //                 {LabelVin, broadcastItem.Vin},
            //                 {LabelSku, palletItem.Sku},
            //                 {LabelDate, now.ToString("MM/dd/yyyy")},
            //                 {LabelSeatDescription, $"{program} {row} SEATS"},
            //                 {LabelRotation, broadcastItem.RotationText}
            //             };

            throw new NotImplementedException("LoadLabelFormatter.Format() is not yet impemented!");
